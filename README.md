...
- Các dạng phân loại: `code`, `sub_code`, `sub_category`, `horizon`,
...
Các notebook sử dụng dữ liệu dạng parquet (ví dụ trên Kaggle):
...
Cho từng quan sát theo thời gian, dự đoán:
$$
\hat{y} = f(\text{features}, \text{categorical context}, \text{history})
$$

Do dữ liệu có trọng số `weight`, quá trình tối ưu và đánh giá được thiết kế để ưu tiên những quan sát quan trọng hơn.

---

## 2. Tiền xử lý dữ liệu
Tiền xử lý được triển khai theo 2 hướng trong hai notebook (mang tính thử nghiệm/so sánh). Pipeline dùng trong mô hình Transformer ở `main.ipynb` là hướng **Label Encoding + Embedding**.
...
- Nhóm rỗng nhiều: `null_ratio >= 0.05` → tạo thêm cờ thiếu (missing indicator) và vẫn điền bằng **median**.

Trong `main.ipynb`, ý tưởng tương tự được áp dụng cho danh sách `stable_num_features`: nếu một cột có tỷ lệ thiếu ≥ 5% thì tạo thêm cột `*_missing_flag` (float32), sau đó điền thiếu bằng median.
...
1) `Data preprocessing.ipynb`: one-hot cho `sub_category`, `horizon` bằng `get_dummies`, và cast `code`, `sub_code` sang `category`.
2) `main.ipynb` (pipeline chính): `LabelEncoder` cho `code`, `sub_code`, `sub_category`, `horizon` sau khi fit trên **hợp** train+test để tránh unseen category.

Hướng (2) phù hợp cho embedding trong Transformer.
...
Với `max_seq_len = 90`, mô hình học dựa trên 90 bước thời gian gần nhất cho mỗi `code`.

---

## 5. Mô hình hoá
### 5.1. Dataset dạng sequence
Lớp `HedgeFundDataset` (PyTorch) tạo mẫu đầu vào dạng chuỗi độ dài `seq_len`. Một index hợp lệ là index $t$ sao cho toàn bộ cửa sổ $[t-seq\_len+1,\dots,t]$ thuộc cùng một `code`.
...
- Early stopping.
- Dự đoán test được “damp” lại (`prediction * 0.5`) như một heuristic để giảm biên độ.
...
- Số epoch: 4 (mang tính demo/quick run)
...
### 9.2. Dữ liệu
Các notebook hiện đang trỏ tới đường dẫn kiểu Kaggle như:

- `/kaggle/input/competitions/ts-forecasting/train.parquet`
- `/kaggle/input/competitions/ts-forecasting/test.parquet`

Nếu chạy local, hãy sửa các đường dẫn này sang vị trí dữ liệu của bạn.
...
- **Validation nâng cao**: dùng walk-forward validation hoặc nhiều mốc split theo thời gian.
- **Tối ưu hoá mô hình**: tune `embed_dim`, số layer/head, regularization, learning rate schedule.
- **Ablation study**: kiểm chứng đóng góp của AutoEncoder features và nhánh wide.
- **Ensembling**: kết hợp Transformer + LightGBM.
- **Ổn định suy luận**: xử lý cold-start cho `code` hiếm và kiểm soát distribution shift.